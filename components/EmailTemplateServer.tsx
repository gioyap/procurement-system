...
const EmailTemplateServer: React.FC<EmailTemplateProps> = ({ data }) => {
	console.log("Email Template Data:", data);
	const approvalLink = `${process.env.NEXT_PUBLIC_BASE_URL}/admin/approve?id=${data.id}`;
	const rejectLink = `${process.env.NEXT_PUBLIC_BASE_URL}/admin/reject?id=${data.id}`;
	...
	return (
		...
		>
			...
			<p style={{ fontSize: "18px", marginBottom: "20px" }}>
				You have received a new procurement request. Here are the details:
			</p>
			<table
				border={1}
				cellPadding="10"
...
export default EmailTemplateServer;
